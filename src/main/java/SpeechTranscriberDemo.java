...
public class SpeechTranscriberDemo {
    ...
    private static final Map<String,StringBuffer> sbMap = new HashMap<String, StringBuffer>();
    ...
    public SpeechTranscriberDemo(String appKey, String id, String secret, String url) {
        ...
        try {
            accessToken.apply();
            System.out.println("get token: " + accessToken + ", expire time: " + accessToken.getExpireTime());
            // TODO 创建NlsClient实例,应用全局创建一个即可,用户指定服务地址
            if (url.isEmpty()) {
                client = NlsClientFactory.getClient();
            } else {
                client = NlsClientFactory.getClient();
            }
        } catch (IOException e) {
    ...
    private static SpeechTranscriberListener getTranscriberListener(final String fileName) {
...
//                        //当前已处理的音频时长，单位是毫秒
//                        ", time: " + response.getTransSentenceTime());
                System.out.println(fileName + "->" + response.getTransSentenceText());
                if(!sbMap.containsKey(fileName)){
                    sbMap.put(fileName,new StringBuffer());
                }
                sbMap.get(fileName).append(response.getTransSentenceText());
            }
...
//                System.out.println("task_id: " + response.getTaskId() + ", status: " + response.getStatus() + ", status_text: " + response.getStatusText());
    ...
    public void process(String filepath) {
        ...
        try {
            //创建实例,建立连接
            transcriber = new SpeechTranscriber(client, getTranscriberListener(filepath));
    ...
    public void shutdown() {
        client.shutdown();
        for(Map.Entry<String,StringBuffer> entry:sbMap.entrySet()){
            System.out.println(entry.getKey() + "\t" + entry.getValue());
        }
    }

    public static void main(String[] args) throws Exception {
        String appKey = "6IiNqbuvYmOqJE77";
        String id = "vC2cUveCZLHoaakg";
        String secret = "follNRRvhwx2Fvg7NgMOZ0FKSy1LRy";
        ...
        File dir = new File("/home/channing/Downloads/20通录音/transiber");
        SpeechTranscriberDemo demo = new SpeechTranscriberDemo(appKey, id, secret, url);
        if(dir.isDirectory()){
            File[] files = dir.listFiles();
            for(File file:files){
                demo.process(file.getAbsolutePath());
            }
        }
        demo.shutdown();
    }
